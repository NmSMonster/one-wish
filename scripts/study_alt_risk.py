...
def main() -> None:
    ...
    for asset in ASSETS:
        ...
        a = assess_short_liquidation(highs, interval=args.interval, window=args.window,
                                     leverages=levs, mmr=args.mmr, symbol=symbol)
        rows.append((asset.value, a))

    lev_hdr = " ".join(f"{l:.0f}x" for l in levs)
    header = f"{'asset':6} {'bary':>5} {'najg.1bar':>10} {'najg.okno':>10}  likwidacja? [{lev_hdr}]"
    print(f"RYZYKO LIKWIDACJI SHORT — realne ceny (interwał {args.interval}, okno reakcji "
          f"{args.window} bar, mmr {args.mmr:.0%})\n")
    print(header)
    print("-" * len(header))
    lines = [header, "-" * len(header)]
    for name, a in rows:
        flags = " ".join(("LIKW" if a.breaches[l]["liquidated"] else "ok").ljust(4) for l in levs)
        line = (f"{name:6} {a.n_bars:5d} {a.worst_1bar_up:9.1%} {a.worst_window_up:9.1%}  {flags}")
        print(line)
        lines.append(line)

    print("\nPrógi likwidacji (ruch w górę): " +
          ", ".join(f"{l:.0f}x → +{(1/l - args.mmr) * 100:.0f}%" for l in levs))
    print("LIKW = historyczny ruch ceny w górę przekroczył próg → short byłby zlikwidowany.")
    print("Jeden taki ruch krystalizuje stratę większą niż funding zbierał miesiącami.")

    _write_doc(lines, levs, args)


def _write_doc(lines: list[str], levs, args) -> None:
    import datetime
    today = datetime.date.today().isoformat()
    thresholds = ", ".join(f"{l:.0f}x → +{(1 / l - args.mmr) * 100:.0f}%" for l in levs)
    doc = f"""# RYZYKO LIKWIDACJI — realna historia cen

> Wygenerowane {today} przez `scripts/study_alt_risk.py`. Interwał {args.interval},
> okno reakcji {args.window} bar, maintenance margin {args.mmr:.0%}.

```
{chr(10).join(lines)}
```

Progi likwidacji (ruch ceny w górę): {thresholds}
